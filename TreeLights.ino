...
// D1 Mini pin definitions
#define IN1_PIN D1    // GPIO5
#define IN2_PIN D2    // GPIO4
#define ENA_PIN D5    // GPIO14 (PWM)
#define MODE_BUTTON D7 // Push button for mode selection
...
{
  Serial.begin(9600);
  Serial.println("Christmas Lights Controller Booting...");
...
void meteor() {
  ...
  if (currentMillis - lastUpdate > 50) {
    ...
    if (animationStep < 10) {
      // Meteor on set A
      direction = 1;
      brightness = (animationStep < 5) ? 
                  (animationStep * 50) : 
                  (255 - (animationStep - 5) * 50);
    } else {
      // Meteor on set B
      direction = -1;
